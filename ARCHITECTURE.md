...
## 二、系统架构总览（Nautilus Trader 整合后）

### 2.0 架构演进说明

**重要变化**：系统已整合 **Nautilus Trader 1.204.0** 作为核心交易引擎，原有的分层架构被整合到 Nautilus 内部，同时保留 Redis Streams 作为与外部系统（LLM 层、API 网关、前端）的桥接。

**核心优势**：
- ✅ **专业级引擎**：使用经过充分测试的机构级交易引擎
- ✅ **回测=实盘**：完全相同的策略代码用于回测和实盘
- ✅ **性能优化**：Rust 核心 + Python API，兼顾性能和开发效率
- ✅ **零修改前端**：通过 RedisBridgeActor 保持消息格式兼容
...
#### 2.2.1 为什么整合 Nautilus？

| 原因 | 说明 |
|------|------|
| **专业级引擎** | 避免重复造轮子，使用经过充分测试的交易引擎 |
| **回测=实盘** | 完全相同的策略代码，消除回测偏差 |
| **性能优化** | Rust 核心提供纳秒级延迟 |
| **生态丰富** | 支持多交易所、多资产类型 |
| **持续维护** | 活跃的开源社区，持续更新 |

#### 2.2.2 为什么保留 Redis Streams？

| 原因 | 说明 |
|------|------|
| **外部系统桥接** | LLM 层、API 网关、前端需要独立通信 |
| **前端零修改** | 保持消息格式兼容，前端无需改动 |
| **审计日志** | 所有事件持久化到 Redis 用于审计 |
| **灵活扩展** | 可以轻松添加新的外部组件 |
