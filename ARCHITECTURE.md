...
┌─────────────────────────────────────────────────────────────────────────┐
│                         数据获取层 · Rust                                │
│    Connector trait · WebSocket/REST适配器 · 数据规范化 · 时间序列环形缓冲   │
└─────────────────────────────────────────────────────────────────────────┘
                               │
                               ▼ NormalizedTick
┌─────────────────────────────────────────────────────────────────────────┐
│                            消息总线                                      │
│            Redis Streams / Kafka · topic扇出 · 事件持久化                │
└─────────────────────────────────────────────────────────────────────────┘
                               │
            ┌──────────────────┴──────────────────┐
            ▼                                      ▼
┌──────────────────────────┐        ┌───────────────────────────────────┐
│      策略层 · Python      │        │           LLM层 · Python           │
│  插件化策略 · 回测引擎     │◄───────│  GPT-4o · Claude · Ollama          │
│  信号合成器 · 热重载       │  权重  │  新闻情感 → 权重向量              │
└────────────┬─────────────┘        └───────────────────────────────────┘
             │
             ▼ 信号
┌─────────────────────────────────────────────────────────────────────────┐
│                        风险管理层 · Go                                   │
│     仓位限制 · 熔断器 · 最大回撤控制 · 实时P&L监控                        │
└─────────────────────────────────────────────────────────────────────────┘
             │
             ▼ 风控通过
┌─────────────────────────────────────────────────────────────────────────┐
...
└─────────────────────────────────────────────────────────────────────────┘
             ...
             ▼ 订单执行
┌─────────────────────────────────────────────────────────────────────────┐
│                        展示层 · Vite+React / Next.js                   │
│  实时行情 · 策略编辑器 · 订单簿 · 投资组合 · 回测报告                      │
└─────────────────────────────────────────────────────────────────────────┘
```

### 2.1 Nautilus Trader 整合架构

**重要更新**：系统已整合 Nautilus Trader 作为核心交易引擎，同时保留 Redis Streams 作为与外部系统的桥接层。

#### 架构映射关系

| 原有组件 | 整合后 | 说明 |
|---------|--------|------|
| Rust 数据层 | **Nautilus DataEngine** | 使用 Nautilus 的 Rust 核心数据引擎 |
| Python 策略层 | **Nautilus Strategy + CrazytraStrategy** | 继承 Nautilus Strategy，添加 LLM 支持 |
| Go 风控层 | **Nautilus RiskEngine** (可选保留自建) | 使用 Nautilus 风控或混合模式 |
| Go 交易层 | **Nautilus ExecutionEngine** | 使用 Nautilus 执行引擎 |
| Redis Streams | **保留** | 作为 Nautilus→外部系统的桥接 |
| LLM 层 | **保持独立** | 继续写入 `llm.weight` topic |
| API 网关 | **不变** | 继续订阅 Redis |
| 前端 | **完全不变** | 零修改 |

#### 核心桥接组件

**RedisBridgeActor** (`nautilus-core/actors/redis_bridge.py`)
- 订阅 Nautilus 所有 tick 和订单事件
- 转换为与自建数据层完全相同的 JSON 格式
- 写入 Redis Streams（`market.tick.*`, `order.event`）
- 使用 `asyncio.create_task()` 非阻塞发布
- 确保前端和 API 网关零修改
...
- 继承 Nautilus `Strategy`（保持回测=实盘特性）
- 添加 LLM 权重支持
- 提供 `get_effective_llm_factor()` 方法
- 支持热重载（`export_state`/`import_state`）

#### 数据流示意图

```
外部市场（Binance / Polymarket）
            ↓  WebSocket / REST
     ...
     [Redis Streams: market.tick.*]
            ↓                    ↓
  [Nautilus Strategy]    [API 网关 · Go]
  + CrazytraStrategy            ↓
            ↓              [前端 · React]
  [LLMWeightActor] ← [Redis: llm.weight] ← [LLM 层 · Python]
            ↓
  Strategy.on_llm_weight_updated()
            ↓  调整仓位
  [Nautilus RiskEngine]
            ↓  风控通过
...
1. **消息格式**：Redis 消息格式与 SYSTEM_SPEC.md 4.5 节完全一致
2. **Symbol 转换**：`BTCUSDT.BINANCE` → `BTC-USDT`
3. **订单状态映射**：Nautilus 状态 → 自建格式
4. **前端零修改**：所有 WebSocket 消息格式保持不变
