# Crazytra 自动交易系统 - 完整安装指导

本文档提供 Crazytra 系统的完整安装和配置指南，包括 Nautilus Trader 整合后的所有组件。
...
docker run -d \
  --name crazytra-redis \
  -p 6379:6379 \
  -v crazytra-redis-data:/data \
  redis:7.2-alpine \
  redis-server --appendonly yes

# 验证 Redis 运行
docker logs crazytra-redis
...
```bash
git clone https://github.com/your-org/Crazytra.git
cd Crazytra
```
...
# 初始化 Go 模块（如果还没有）
go mod init github.com/your-org/crazytra/api-gateway
...
创建 `/etc/systemd/system/crazytra-nautilus.service`：

```ini
[Unit]
Description=Crazytra Nautilus Trading Node
After=network.target redis.service

[Service]
Type=simple
User=crazytra
WorkingDirectory=/opt/crazytra/nautilus-core
Environment="PATH=/opt/crazytra/venv/bin"
ExecStart=/opt/crazytra/venv/bin/python main.py --mode live
Restart=always
RestartSec=10
...
sudo systemctl enable crazytra-nautilus
sudo systemctl start crazytra-nautilus
sudo systemctl status crazytra-nautilus
```
...
构建和运行：
```bash
docker build -t crazytra-nautilus .
docker run -d \
  --name crazytra-nautilus \
...
- **问题**: [GitHub Issues](https://github.com/your-org/Crazytra/issues)
- **社区**: [Discord / Telegram]
